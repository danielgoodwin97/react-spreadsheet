...
function equals(a, b) {
    ...
    var _b = getSize(a), rowsA = _b.rows, columnsA = _b.columns;
    var _c = getSize(b), rowsB = _c.rows, columnsB = _c.columns;
    if (rowsA !== rowsB || columnsA !== columnsB) {
        return false;
    }
    try {
        for (var _d = __values(entries(a)), _e = _d.next(); !_e.done; _e = _d.next()) {
            var _f = __read(_e.value, 2), point = _f[0], valueA = _f[1];
            var valueB = get(point, b);
            if (valueA !== valueB) {
                if (!valueA || !valueB) {
                    return false;
                }
...
var Spreadsheet = function (props) {
    ...
    React__namespace.useEffect(function () {
        ...
        if (props.selected &&
            ...
            !((_a = prevSelectedPropRef.current) === null || _a === void 0 ? void 0 : _a.equals(props.selected))) {
            setSelection$1(props.selected);
        }
        prevSelectedPropRef.current = props.selected;
    }, [props.selected, setSelection$1, state.selected]);
    ...
    React__namespace.useEffect(function () {
        if (props.data !== prevDataPropRef.current &&
            !equals(props.data, state.model.data)) {
            setData$1(props.data);
        }
        prevDataPropRef.current = props.data;
    }, [props.data, setData$1, state.model.data]);
